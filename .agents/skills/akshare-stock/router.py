...
TRADING_SIGNAL = "TRADING_SIGNAL"  # 交易信号
...
def _classify_intent(query: str) -> str:
    ...
    # 交易信号
    if any(k in query for k in ["交易信号", "买卖信号", "技术指标", "金叉", "死叉", "MACD", "RSI", "KDJ", "突破", "买入", "卖出"]):
        return TRADING_SIGNAL
    
    # 模拟盘
    if any(k in query for k in ["模拟盘", "虚拟交易", "回测", "模拟买入", "模拟卖出"]):
        return PAPER_TRADE
    
    # 价格预警
    if any(k in query for k in ["预警", "提醒", "监控", "突破提醒", "跌破提醒"]):
        return ALERT
